...
function ViewChallans() {
  ...
  const getFilteredChallans = () => {
    ...
    if (filterQuality !== 'all') {
      filtered = filtered.filter(c => c.quality._id === filterQuality);
    }
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            >
              Incomplete ({challans.filter(c => c.status === 'incomplete' && !c.isSold).length})
            </button>
            <button
              onClick={() => setFilterStatus('complete')}
              className={`btn ${filterStatus === 'complete' ? 'btn-primary' : 'btn-secondary'} btn-small`}
            >
              Complete ({challans.filter(c => c.status === 'complete' && !c.isSold).length})
            </button>
            <button
              onClick={() => setFilterStatus('sold')}
              className={`btn ${filterStatus === 'sold' ? 'btn-primary' : 'btn-secondary'} btn-small`}
            >
              Sold ({challans.filter(c => c.isSold).length})
            </button>
          </div>
        </div>
      </div>
