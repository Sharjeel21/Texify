...
router.get('/next-bill-number', async (req, res) => {
  try {
    const lastInvoice = await TaxInvoice.findOne().sort({ billNumber: -1 });
    ...
    for (let invoice of invoices) {
      ...
      if (invoiceObj.challanDetails && invoiceObj.challanDetails.length > 0) {
        for (let i = 0; i < invoiceObj.challanDetails.length; i++) {
          ...
          const challan = await DeliveryChallan.findById(challanDetail.challanId);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (dealId) {
      ...
      actualRate = deal.ratePerMeter; // Use deal rate
    } else {
      // Manual invoice creation (existing flow)
      partyDoc = await Party.findById(party).session(session);
      qualityDoc = await Quality.findById(quality).session(session);
    ...
    if (challans.length === 0) {
      await session.abortTransaction();
      return res.status(400).json({ message: 'No challans selected' });
    }
    ...
    if (dealId && deal) {
      ...
      // Each invoice = 1 completed bilty (regardless of how many challans)
      // Progress is based on number of invoices created
      deal.completedBilties = deal.invoiceIds.length;
      
      // Also link the challans to the deal for reference
      challans.forEach(challan => {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Fetch bale details for each challan
    if (invoiceObj.challanDetails && invoiceObj.challanDetails.length > 0) {
      for (let i = 0; i < invoiceObj.challanDetails.length; i++) {
        const challanDetail = invoiceObj.challanDetails[i];
        const challan = await DeliveryChallan.findById(challanDetail.challanId);
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const invoice = await TaxInvoice.findById(req.params.id).session(session);
    ...
    // Remove from deal if associated and recalculate progress
    if (invoice.dealId) {
      const deal = await Deal.findById(invoice.dealId).session(session);
      
      if (deal) {
        // Remove invoice from deal
        deal.invoiceIds = deal.invoiceIds.filter(
          id => id.toString() !== invoice._id.toString()
        );
        
        // Recalculate progress based on invoice count
        deal.completedBilties = deal.invoiceIds.length;
    ...
    await TaxInvoice.findByIdAndDelete(req.params.id).session(session);
